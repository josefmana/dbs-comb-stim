...
# ONE PARTICIPANT/LOT OF DATA ----

d1 <- ssrt_data_sim( N = 1, K = c(500,500) )

summary(d1$data$rt)
sum(d1$data$rt < 0, na.rm = T) / nrow(d1$data)
...
# fitting proper
fit1 <- mod_indi$sample(data = dlist, chains = 4, save_warmup = T, init = ifun)

mcmc_trace( fit1$draws(inc_warmup = T), n_warmup = 1e3 )
      ...
      function(j) {
        ...
        return( with(
          ...
          ssrt_data_sim(
            ...
            epsilon_stop = c(0, 0),
            N = 1,
            df = subset(d1$data, id == i) %>% mutate(id = 1, rt = NA)
          )$data$rt
